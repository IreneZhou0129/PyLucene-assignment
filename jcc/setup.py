...
INCLUDES = {
    ...
    'darwin/home': ['%(darwin)s/Contents/Home/include' %(JDK),
                    '%(darwin)s/Contents/Home/include/darwin' %(JDK)],
...
LFLAGS = {
    ...
    'darwin/home': ['-L%(darwin)s/Contents/Home/lib' %(JDK), '-ljava',
                    '-L%(darwin)s/Contents/Home/lib/server' %(JDK), '-ljvm',
                    '-Wl,-rpath', '-Wl,%(darwin)s/Contents/Home/lib' %(JDK),
                    '-Wl,-rpath', '-Wl,%(darwin)s/Contents/Home/lib/server' %(JDK),
                    '-mmacosx-version-min=10.9'],
